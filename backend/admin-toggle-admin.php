...
if (!empty($userID)) {
    ...
    $msg = $conn->prepare("SELECT admin FROM users WHERE user_id = ?");

    if ($msg) {    
	$msg->bind_param("i", $userID);
	$msg->execute();
	$msg->bind_result($admin_status);
	...
	if($msg->fetch()) {
		...
		if ($admin_status == 1) {
			// flip to inactive
			$msg2 = $conn->prepare("UPDATE users SET admin = 0 WHERE user_id = ?");
			...
			$msg2 = $conn->prepare("UPDATE users SET admin = 1 WHERE user_id = ?");
			$msg2->bind_param("i", $userID);
			$msg2->execute();
			$msg2->close();
