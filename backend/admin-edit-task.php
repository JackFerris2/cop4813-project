...
if (!empty($taskID) && !empty($userID) && !empty($taskTitle) && !empty($taskDescription) && !empty($taskStatus)) {
    ...
    // add task
    $msg = $conn->prepare("UPDATE tasks SET user_id = ?, title = ?, description = ?, status = ? WHERE task_id = ?");

    if ($msg) {
        $msg->bind_param("isssi", $userID, $taskTitle, $taskDescription, $taskStatus, $taskID);
